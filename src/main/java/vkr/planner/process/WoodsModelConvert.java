...
import vkr.planner.model.specification.woods.*;
...
public class WoodsModelConvert {
    ...
    public TechnicalDescriptionWoods convertMapToTechnicalDescriptionWoods(){
            ...
            while (!stop){
                ...
                if (num.isPresent()){
                    area.getPipeList().add(new Pipe(
                            Double.parseDouble(excelTable.get(key + 1).get(COORDINATES_INDEX)),
                            Double.parseDouble(excelTable.get(key + 2).get(COORDINATES_INDEX)),
                            Double.parseDouble(excelTable.get(key + 3).get(COORDINATES_INDEX)),
                            isNeedToBeThermallyTnsulated(excelTable.get(key).get(THERMALLY_INSULATED_INDEX)),
                            kks.orElse(KKS_NOT_FOUND)
                    ));
                    key += 3;
                }
                else stop = true;
